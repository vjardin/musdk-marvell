...
static int txpause_enable_cmd_cb(void *arg, int argc, char *argv[])
{
	int enable = -1, portid = 0, port_set = 0;
	struct glb_common_args *glb_args = (struct glb_common_args *) arg;
	struct port_desc *port_desc = ((struct pp2_glb_common_args *) glb_args->plat)->ports_desc;
	struct pp2_ppio_tx_pause_params tx_p_params  = {0};
	char *ret_ptr;
	int option = 0;
	int long_index = 0;
	struct option long_options[] = {
	...
	if (enable != -1) {
		tx_p_params.en = enable;
		pp2_ppio_set_tx_pause(port_desc[portid].ppio, &tx_p_params);
	} else {
		printf("pp2_ppio_get_tx_pause not implemented n");
	}
...
int app_register_cli_common_cmds(struct glb_common_args *glb_args)
{
	...
	cmd_params.name		= "txpause";
	cmd_params.desc		= "tx pause enable/disable";
	cmd_params.format	= "--port --enable\n"
				  "\t\t--port, -p	port number, required param\n"
				  "\t\t--enable, -e	0-disable, 1-enable, show current if unspecified\n";
	cmd_params.cmd_arg	= glb_args;
	cmd_params.do_cmd_cb	= (int (*)(void *, int, char *[]))txpause_enable_cmd_cb;
	mvapp_register_cli_cmd(&cmd_params);
